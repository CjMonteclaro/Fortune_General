 class MotorsReport < Prawn::Document
      ...
      def transaction_rows
           ...
           @motor_policies = Policy.where(acct_ent_date: @start_date..@end_date).where(line_code: "MC").where(subline_code: ["PC","CV"]).or(Policy.where(spld_acct_ent_date: @start_date..@end_date).where(line_code: "MC").where(subline_code: ["PC","CV"])).includes(:item, :item_perils, :perils, :vehicle, :mc_car_company, :type_of_body).map do |l|

            @motor_pol = l.perils&.where(line_code: "MC").each do |peril|
            subtable = make_table(peril&.shortname)
            @motor_po = peril.item_perils&.where(peril_cd: peril).each do |item|
              subtable2 = make_table([[item&.proper_tsi], [item&.proper_prem], [item.proper_rate]])
              [l&.policy_no, l&.endorsemnt, l&.iss_date, l&.ef_date, l&.exp_date, l.vehicle&.vehicle_name, subtable, subtable2 ]
             end
           end
        end
      end
