...
class MotorsController < ApplicationController
  ...
  #  @motor_policies_csv = Motorpolicy.where(acct_ent_date: @start_date..@end_date).where(line_code: "MC").or(Motorpolicy.where(spld_acct_ent_date: @start_date..@end_date)).includes(:item, :item_perils, :perils, :vehicle, :mc_car_company, :type_of_body)
